...
void mwFont::load_fonts(void)
{
   ...
   if (!bltn)
   {
      mInput.m_err("Failed to create_builtin_font");
      exit(0);
   }
   ...
   if (!sauc) mInput.m_err("Failed to load font from bitmaps/SaucerBB.ttf");
...
void mwFont::mw_get_text_dimensions(ALLEGRO_FONT *f, const char* txt, int &bx, int &by, int &bw, int &bh)
{
   ...
   if (f == pixl) { by = 4; bh = 5; }
   if (f == pr8)  { by = 0; bh = 8; bx = 0; bw = strlen(txt) * 8; }
   if (f == bltn) { by = 0; bh = 8; }
}
...
void mwFont::convert_ttf_to_bitmap_font(const char* ttf_filename, const char* bmp_filename, int char_size)
{
   ...
   if (!cf)
   {
      ...
      mInput.m_err("Failed to load font from bitmaps/Pristine.ttf");
   }
