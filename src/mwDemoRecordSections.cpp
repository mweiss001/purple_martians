...
void mwDemoRecord::proc_section_details_menu()
{
   if (mInput.mouse_b[2][0])
   {
      ...
      if (mp == 4 && p != 0) delete_section(i); // don't delete player 0
...
void mwDemoRecord::draw_current_section(mwWindow w)
{
   ...
   // write the gmInfo name to the player struct
   sprintf(mPlayer.syn[p].name,"%s", mGmInfo.gmPlayerInfo[i].playerName.c_str());
   ...
   ya+=line_space;
   al_draw_textf(mFont.pr8, mColor.pc[15], x, ya, 0, "Enemy Hits:%d", (int)mGmInfo.gmPlayerInfo[i].enemyHits.size());
...
{
   mwRect<int> rect = mwRect<int>::fromX1Y1WH(mInput.mouse_x, mInput.mouse_y, 240, 118);
   ...
   // set player number to first item in list (if any in list)
   if (listItems.size()) player_num = listItems[0].value;

   // if no items in list, reduce the dialog height
   else rect.setHeight(52);
   ...
   while (!quit)
   {
      ...
      if (listItems.size() == 0)
      {
         al_draw_text(mFont.pr8, mColor.pc[15], xa, ya, 0, "No unused player numbers!");
         ya+=12+mLoop.pct_y; al_draw_line(rect.x1, ya, rect.x2, ya, mColor.pc[c], 1); ya+=4;
      }
      else
      {
         al_draw_text(mFont.pr8, mColor.pc[15], xa, ya+4, 0, "Player Number:");
         mWidget.mDropDown(1, xa+114, 90,  1, ya, bts, 0, 0, 0, 0, 15, 10, listItems, player_num, 0);

         ya+= bts+4; al_draw_line(rect.x1, ya, rect.x2, ya, mColor.pc[c], 1); ya+=4;

         al_draw_text( mFont.pr8, mColor.pc[15], xa, ya+2, 0, "Player Name:");
         if (mWidget.buttontcb(xa+98, ya, 0, 13,  0,0,0,0,  0,15,15,10,  1,0,0,0, mPlayer.syn[player_num].name)) mMiscFnx.edit_player_name(xa+134, ya, player_num);
