...
int is_down_solidf(al_fixed x, al_fixed y, int lift_check)
{
   int a, b=0, c=0, ret = 0;
   int yy = (al_fixtoi(y)-1) / 20 + 1;
   int cc = al_fixtoi(x) / 20;
   int xx = al_fixtoi(x) / 20 + 1;
   a = al_fixtoi(x) % 20;
   if (a == 0)    // this block only
   {
      c = l[cc][yy];
...
int is_up_solidf(al_fixed x, al_fixed y, int lift_check)
{
   int a, b, c;

   int yy = al_fixtoi(y) / 20;
   int cc = al_fixtoi(x) / 20;
   int xx = al_fixtoi(x) / 20 + 1;
   a = al_fixtoi(x) % 20;
   if (a == 0)    // this block only
...
int is_right_solidf(al_fixed x, al_fixed y, int lift_check)
{
   int a, b, c;

   int xx = (al_fixtoi(x)-1) / 20 + 1;
   int yy = al_fixtoi(y) / 20;
   int cc = al_fixtoi(y) / 20 + 1;
   a = al_fixtoi(y) % 20;
   if (a == 0)    // this block only
   {
      c = l[xx][yy];
...
int is_left_solidf(al_fixed x, al_fixed y, int lift_check)
{
   int a, b, c;

   int xx = al_fixtoi(x) / 20;
   int yy = al_fixtoi(y) / 20;
   int cc = al_fixtoi(y) / 20 + 1;
   a = al_fixtoi(y) % 20;
   if (a == 0)    // this block only
...
void solid_test(void)
{
   ...
   speed = 5;
   set_speed();
   players[0].PX = al_itofix(1000);
   players[0].PY = al_itofix(1000);
   while (!quit)
   {
      proc_controllers();
...
// original solid functions
      if (players[0].up)
      {
         players[0].PY -= al_itofix(1);
         if (is_up_solid(al_fixtoi(players[0].PX), al_fixtoi(players[0].PY), 0)) players[0].PY += al_itofix(1);
      }
      if (players[0].down)
      {
         players[0].PY += al_itofix(1);
         if (is_down_solid(al_fixtoi(players[0].PX), al_fixtoi(players[0].PY), 0)) players[0].PY -= al_itofix(1);
      }
      if (players[0].right)
      {
         players[0].PX += al_itofix(1);
         if (is_right_solid(al_fixtoi(players[0].PX), al_fixtoi(players[0].PY), 0)) players[0].PX -= al_itofix(1);
      }
      if (players[0].left)
      {
         players[0].PX -= al_itofix(1);
         if (is_left_solid(al_fixtoi(players[0].PX), al_fixtoi(players[0].PY), 0)) players[0].PX += al_itofix(1);
      }
...
/*
      ...
      {
         players[0].PY -= al_itofix(1);
         if (is_up_solidf(players[0].PX, players[0].PY, 0)) players[0].PY += al_itofix(1);
      }
      if (players[0].down)
      {
         players[0].PY += al_itofix(1);
         if (is_down_solidf(players[0].PX, players[0].PY, 0)) players[0].PY -= al_itofix(1);
      }
      if (players[0].right)
      {
         players[0].PX += al_itofix(1);
         if (is_right_solidf(players[0].PX, players[0].PY, 0)) players[0].PX -= al_itofix(1);
      }
      if (players[0].left)
      {
         players[0].PX -= al_itofix(1);
         if (is_left_solidf(players[0].PX, players[0].PY, 0)) players[0].PX += al_itofix(1);
      }
...
// new solid functions with move
      ...
      rect(level_buffer, al_fixtoi(players[0].PX), al_fixtoi(players[0].PY), al_fixtoi(players[0].PX)+19, al_fixtoi(players[0].PY)+19, palette_color[10]);
