...
void mwLevelEditor::init_windows()
{
   ...
   mWM.mW[4].set_title("selection edit");
   ...
   mWM.mW[6].set_title("ge controls");
...
bool mwLevelEditor::get_new_box(const char* text, bool preview)
{
...
void mwLevelEditor::process_mouse()
{
   ...
   {
      if (mode == 1) mEditorMain.process_mouse();
      if (mode == 2) mEditSelection.process_mouse();
      if (mode == 3) mGroupEdit.process_mouse();
      if (mode == 4) mObjectViewer.ov_process_mouse();
      if (mode == 9) mTileHelper.process_mouse();
   }
}
...
void mwLevelEditor::redraw_background(int mode)
{
   int old_lem = mode;
   mode = mode;
   redraw_background();
