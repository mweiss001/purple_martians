...
void ov_draw_buttons(int x1, int y1, int x2, int y2, int d)
{
   ...
   if (obt == 4) // lifts
   {
      ...
      if (mwWM.mW[7].mb == 26)
      {
         ...
         if (edit_lift_name(lift, yld, xa+10, fst)) strcpy(mLift.cur[lift].lift_name, fst);
      }
   }
   ...
   if (obt == 2) // items
   {
      switch (type) // item subtypes
      {
         ...
         case 4: // key
            ...
            ya+=4; // spacer
            if (mdw_buttont(xa, ya, xb, bts, 0,0,0,0,   0,10,15, 0,  1,0,1,d, "Get New Block Range")) get_block_range("Block Range", &mItem.item[n][6], &mItem.item[n][7], &mItem.item[n][8], &mItem.item[n][9], 1);
            ya+=4; // spacer
         ...
         case 10: // message
            ...
            if (mdw_buttont(xa, ya, xb, bts, 0,0,0,0,   0,14,15,0,  1,0,1,d, "Set Trigger Area")) get_block_range("Trigger Area", &mItem.item[n][6], &mItem.item[n][7], &mItem.item[n][8], &mItem.item[n][9], 1);
            ya+=4; // spacer
         ...
         case 9: // trigger
         {
            if (mdw_buttont(xa, ya, xb, bts, 0,0,0,0,    0,14,15,0,   1,0,1,d, "Get New Trigger Field")) get_block_range("Trigger Rectangle", &mItem.item[n][6], &mItem.item[n][7], &mItem.item[n][8], &mItem.item[n][9], 1);
            ya+=4; // spacer
         ...
         break;
         case 16: // block manip
            if (mdw_buttont(xa, ya, xb, bts, 0,0,0,0,    0,14,15,14,  1,0,1,d, "Get New Block Manip Field")) get_block_range("Block Manip Rectangle", &mItem.item[n][6], &mItem.item[n][7], &mItem.item[n][8], &mItem.item[n][9], 1);
            ya+=4; // spacer
         ...
         case 17: // block damage
         {
            if (mdw_buttont(   xa, ya, xb, bts, 0,0,0,0,   0,10,15,0,  1,0,1,d, "Get New Block Damage Field")) get_block_range("Block Damage Rectangle", &mItem.item[n][6], &mItem.item[n][7], &mItem.item[n][8], &mItem.item[n][9], 1);
            ya+=4; // spacer
   ...
   if (obt == 2)  // items
   {
      ...
      switch (type)
      {
         ...
         case 10: // pmsg
         {
            ...
            get_int_3216(mItem.item[num][10], mx, my);
            get_int_3216(mItem.item[num][11], mw, mh);
            int x2 = mx;
            int y2 = my;
            int x3 = mx + mw;
            int y3 = my + mh;
         ...
         case 15: // sproingy
         {
            int color = 14;
            if (legend_highlight == 2) color = mC.flash_color;
            int y = get_sproingy_jump_height(num);
...
void ov_process_mouse(void)
{
   ...
   if (mwWM.mW[7].obt == 2)
   {
      ...
      if (type == 10) // pop message
      {
         ...
         int mx=0, my=0, mw=0, mh=0;
         get_int_3216(mItem.item[b][10], mx, my);
         get_int_3216(mItem.item[b][11], mw, mh);
      ...
      if (type == 15) // sproingy
      {
         ...
         int y2 = y1 - get_sproingy_jump_height(mwWM.mW[7].num);
   ...
   if (mI.mouse_b[1][0])
   {
      ...
      while (mI.mouse_b[1][0])
      {
         if (mouse_on_obj)
         {
            ...
            if (mwWM.mW[7].obt == 2) // move item
            {
               ...
               if (mI.SHFT()) // move stuff also
               {
                   ...
                   {
                      int x1=0, y1=0;
                      get_int_3216(mItem.item[n][10], x1, y1);
                      x1 += x_off*20;
                      y1 += y_off*20;
                      set_int_3216(mItem.item[n][10], x1, y1);
                   }
               }
            }
         ...
         {
            set_int_3216(mItem.item[mwWM.mW[7].num][10], mwWM.hx, mwWM.hy);
         }

         if (mouse_on_msg_lr) // move msg
         {
            int mx=0, my=0, mw=0, mh=0;
            get_int_3216(mItem.item[mwWM.mW[7].num][10], mx, my);
            get_int_3216(mItem.item[mwWM.mW[7].num][11], mw, mh);
            ...
            set_int_3216(mItem.item[mwWM.mW[7].num][11], mw, mh);
         ...
         if (mouse_on_sp) // adjust sproingy jump height
         {
            ...
            float y0 = (float) mItem.item[mwWM.mW[7].num][5]+10;
            float fy = (float) mwWM.hy;
            mItem.item[mwWM.mW[7].num][7] = get_sp(y0-fy);
