...
int draw_list_items(int x1, int y1, int flash_color, int ni)
{
   ...
   // valid variable headers
   for (int ge_num=0; ge_num<40; ge_num++)
      if (ge_data[ge_num].valid)
      {
         al_draw_textf(font, palette_color[15], xt, y1, 0, "%.3s", ge_data[ge_num].name);
         xt+=32;
      }
   y1+=8;
...
void show_obj_list(int x, int y, int flash_color)
{
   ...
   al_draw_text(font, palette_color[15], (x1+x2)/2, yf1+2, ALLEGRO_ALIGN_CENTER, "Object List");
...
void show_ge_controls(void)
{
   ...
   if (ni)
   {
      // search the list to find out what controls are valid
      // all on by default
      // removed one by one as soon as the don't apply to all items in list


      ge_data[0].valid = 1; // seek count
      ge_data[1].valid = 1; // seek speed
      ge_data[2].valid = 1; // bullet speed
      ge_data[3].valid = 1; // bullet retrigger
      ge_data[4].valid = 1; // bullet priximity
      ge_data[5].valid = 1; // x speed (arch)
      ge_data[6].valid = 1; // x speed (trak)
      ge_data[7].valid = 1; // y speed
      ge_data[8].valid = 1; // jump before hole
      ge_data[9].valid = 1; // jump before wall
      ge_data[10].valid = 1; // extra hits to kill
      ge_data[11].valid = 1; // drop mode (trakbot)
      ge_data[12].valid = 1; // height above player
      ge_data[13].valid = 1; // flap height
      ge_data[14].valid = 1; // flap speed
      ge_data[15].valid = 1; // x speed (flap)
      ge_data[16].valid = 1; // x accel
      ge_data[17].valid = 1; // initial direction (arch and flap)
      ge_data[18].valid = 1; // initial direction (trak)

      ge_data[30].valid = 1; // initial direction (bouncer and cannon)
      ...
      for (int i=0; i<100; i++)
         if (obj_list[i][0])
         {
            ...
            if (typ == 3) // enemy
            {
               ...
               // seek count (bouncer and cannon)
               if ((etyp != 4) && (etyp != 6)) ge_data[0].valid = 0;

               // seek speed (bouncer and cannon)
               if ((etyp != 4) && (etyp != 6)) ge_data[1].valid = 0;

               // bullet speed (archwagon, cannon, podzilla, trakbot, flapper)
               if ((etyp != 3) && (etyp != 6) && (etyp != 7) && (etyp != 8) && (etyp != 12)) ge_data[2].valid = 0;

               // bullet retrigger (archwagon, cannon, trakbot, flapper)
               if ((etyp != 3) && (etyp != 6) && (etyp != 8) && (etyp != 12)) ge_data[3].valid = 0;

               // bullet proximity (archwagon, trakbot)
               if ((etyp != 3) && (etyp != 8)) ge_data[4].valid = 0;

               // x speed (archwagon)
               if (etyp != 3) ge_data[5].valid = 0;

               // x speed (trakbot)
               if (etyp != 8) ge_data[6].valid = 0;

               // y speed (archwagon, trakbot, flapper)
               if ((etyp != 3) && (etyp != 8) && (etyp != 12)) ge_data[7].valid = 0;

               // jump before hole (archwagon)
               if (etyp != 3) ge_data[8].valid = 0;

               // jump before wall (archwagon)
               if (etyp != 3) ge_data[9].valid = 0;

               // extra hits to kill (cannon)
               if (etyp != 6) ge_data[10].valid = 0;

               // drop mode (trakbot)
               if (etyp != 8) ge_data[11].valid = 0;
               ...
               // initial direction (archwagon and flapper)
               if ((etyp != 3) && (etyp != 12)) ge_data[17].valid = 0;

               // initial direction (trakbot)
               if (etyp != 8) ge_data[18].valid = 0;

               // initial direction (bouncer and cannon)
               if ((etyp != 4) && (etyp != 6)) ge_data[30].valid = 0;
      ...
      for (int ge_num=0; ge_num<40; ge_num++)
         if (ge_data[ge_num].valid)
         {
            ...
            mdw_buttont(x1, y+a*bts, x2, y+(a+1)*bts-2, 0,0,0,0, 0,12,15,0,1,0,0,0, msg); a++;
            ...
            mdw_buttont(x1, y+a*bts, x2, y+(a+1)*bts-2, 0,0,0,0,  0,13,15,0,1,0,0,0, msg); a++;


            if ((gvt == 2) || (gvt == 3) || (gvt == 4))
            {

               mdw_slider2_flt(x1, y+a*bts, x4, y+(a+1)*bts-2, 0,0,0,0, 0,3,15,0,1,0,0,0, ge_data[ge_num].adj_min, ge_data[ge_num].max_allowed, ge_data[ge_num].min_allowed, 1, "Min:");
               mdw_slider2_flt(x4, y+a*bts, x2, y+(a+1)*bts-2, 0,0,0,0, 0,3,15,0,1,0,0,0, ge_data[ge_num].adj_max, ge_data[ge_num].max_allowed, ge_data[ge_num].min_allowed, 1, "Max:"); a++;

               mdw_buttonp(x1, y+a*bts, x3, y+(a+1)*bts-2,   100,0,0,0,  0, 4,15,0,1,1,0,0, ge_data[ge_num].adj_mode); // Action type
               if (mdw_buttont(x3, y+a*bts, x2, y+(a+1)*bts-2, 0,0,0,0,  0,10,15,0,1,0,0,0, "Do It!"))
               {
                  ...
                  if (ge_data[ge_num].adj_mode == 0)
                  {
                     printf("Randomizing %s\n", ge_data[ge_num].name);
                     for (int i=0; i<100; i++) // iterate all items in list
                        if (obj_list[i][0])
                        {
                           int num = obj_list[i][1];
                  ...
                  if (ge_data[ge_num].adj_mode == 2)
                  {
                     printf("Set all to min for %s\n", ge_data[ge_num].name);
                     float val = mn;
                     for (int i=0; i<100; i++) // iterate all items in list
                        if (obj_list[i][0])
                        {
            ...
            {
               if (mdw_buttont(x1, y+a*bts, x4, y+(a+1)*bts-2, 0,0,0,0,  0,10,15,0,1,0,0,0, "Random"))
               {
                  printf("Randomizing %s\n", ge_data[ge_num].name);
                  for (int i=0; i<100; i++) // iterate all items in list
                     if (obj_list[i][0])
               ...
               if (mdw_buttont(x4, y+a*bts, x2, y+(a+1)*bts-2, 0,0,0,0,  0,10,15,0,1,0,0,0, "Aim at Start Block"))
               {
                  ...
                  printf("Aimimng at Start Block %s\n", ge_data[ge_num].name);
                  for (int i=0; i<100; i++) // iterate all items in list
                     if (obj_list[i][0])
                     {
                        int num = obj_list[i][1];
...
//      al_draw_text(font, palette_color[14], stx*db+2, sty*db-11,  0, "selection");
      ...
      if ((mx1 < 100) && (my1 < 100)) // if mouse pointer on map
      {
         ...
         if (mouse_b1)
         {
            ...
            if ((key[ALLEGRO_KEY_LSHIFT]) || (key[ALLEGRO_KEY_RSHIFT])) // draw new selection rectangle
            {
               // initial selection
               stx = mx1;
               sty = my1;
