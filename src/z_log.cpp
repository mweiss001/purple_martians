...
void log_ping_graph(int num_lines)
{
   ...
   if ((mwG[0].calc_data_range()) && (mwG[1].calc_data_range())) run_ping_graph();
}
...
//   for (int i=0; i<1000; i++)
   {
      ...
      if (type == 30) // stak
      {
         ...
         mwG[1].add_data_point(p, (double) fn, dsync);
      }
   }
   ...
   mwG[0].calc_data_range();
   mwG[0].autorange_axis(1, 1);
   mwG[0].set_title("Transmit Rate", 2, sc, sc);             // text, text_color, frame_color
   mwG[0].set_x_axis_legend("Time", "frames", 0, 15, 0);      // text, font, text_color, frame_color
   mwG[0].set_y_axis_legend("Transmit", "kBps", 0, sc, 15);    // text, font, text_color, frame_color
   mwG[0].set_x_axis_labels(1, 1, 2, 15);                    // type, font, tick_size, color
   mwG[0].set_y_axis_labels(1, 1, 2, sc);                    // type, font, tick_size, color


   mwG[0].y_axis_grid_draw_on = 1;
   mwG[0].y_axis_grid_label_draw_on = 1;
   mwG[0].y_axis_scrollbar_draw_on = 1;
   ...
   mwG[1].calc_data_range();
   mwG[1].autorange_axis(1, 1);
   mwG[1].set_title("Client dsync", 2, sc, sc);                 // text, text_color, frame_color
   mwG[1].set_x_axis_legend("Time", "frames", 0, 15, 0);      // text, font, text_color, frame_color
   mwG[1].set_y_axis_legend("dsync", "ms", 0, sc, 0);         // text, font, text_color, frame_color
   mwG[1].set_x_axis_labels(1, 1, 2, 15);                    // type, font, tick_size, color
   mwG[1].set_y_axis_labels(3, 1, 2, sc);                    // type, font, tick_size, color
...
void log_bandwidth_graph(int num_lines, int both)
{
   ...
   if ((mwG[0].calc_data_range()) && (mwG[1].calc_data_range())) run_bandwidth_graph(both);
}
...
void log_client_server_sync_graph(int num_lines)
{
   ...
   if ((mwG[0].calc_data_range()) && (mwG[1].calc_data_range())&& (mwG[2].calc_data_range())) run_client_server_sync_graph();
}
...
void load_profile_graph2(void)
{
   ...
   for (int i=0; i<num_lines; i++)
   {
      ...
      if (type == 44) // tmst
      {
         ...
         if (match > -1)
         {
             mwG[0].add_data_point(match, (double) fn, v);
         }
         else  // no match found
         {
            // find first empty
            ...
            if (match > -1)
            {
               mwG[0].set_series(match, res, 1+match, 0);
               mwG[0].add_data_point(match, (double) fn, v);
            }
         }
         // at this point, if we don't have a match or an empty just do nothing
   ...
   if ((mwG[0].calc_data_range()) && (mwG[0].calc_data_range())) run_profile_graph();
}
