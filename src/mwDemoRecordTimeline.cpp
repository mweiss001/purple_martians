...
void mwDemoRecord::draw_timeline(mwWindow w)
{
   ...
   int gmInfo_index = -1;
   if (draw_timeline_tracks(timeline_rect.x1, timeline_rect.x2, timeline_rect.y1, bts, 2, 0, 0, timeline_rect.w, timeline_rect.h, gmInfo_index, 1, w.disable_input)) set_active_section(gmInfo_index);
}
...
bool mwDemoRecord::draw_timeline_tracks(int x1, int x2, int y1, int bts, int ls, bool smallText, bool sizeOnly, int &w, int &h, int &gmInfo_index, int display_time_labels, int display_only)
{
