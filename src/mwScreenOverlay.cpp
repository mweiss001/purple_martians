...
void mwScreen::sdg_show_column(int col, int &x, int y)
{
   ...
   if (col == 10) // dif src
   {
      ...
      for (int p=0; p<NUM_PLAYERS; p++)
      {
         if (mPlayer.syn[p].active == 1) color = color1;
         if (mPlayer.syn[p].active == 0) color = color2;
         al_draw_textf(mF.pr8, mC.pc[color], x, y+=8, 0, "[%5d]", mNetgame.srv_client_state_frame_num[p][0]);
      }
      x+=7*8;
   }
   if (col == 11) // dif dst
   {
      ...
      for (int p=0; p<NUM_PLAYERS; p++)
      {
         if (mPlayer.syn[p].active == 1) color = color1;
         if (mPlayer.syn[p].active == 0) color = color2;
         al_draw_textf(mF.pr8, mC.pc[color], x, y+=8, 0, "[%5d]", mNetgame.srv_client_state_frame_num[p][1]);
      }
      x+=7*8;
   }
...
void mwScreen::draw_common_debug_overlay(int p, int &cx, int &cy)
{
   ...
   if (mSettings.overlay_grid[1][mLoop.show_debug_overlay]) // display
   {
      ...
      al_draw_textf(mF.pr8, mC.pc[15], cx+1, cy, 0, "frames per second:%d", mLoop.frame_speed); cy+=9;
...
void mwScreen::draw_server_debug_overlay(int p, int &cx, int &cy)
{
   ...
   if (mSettings.overlay_grid[6][mLoop.show_debug_overlay]) // state freq adjust buttons
   {
      if (mPlayer.loc[0].server_state_freq_mode == 0) // 0 = manual, 1 = auto
      {
         ...
         al_draw_textf(mF.pr8, mC.pc[15], csx1+csw/2, csy3+2, ALLEGRO_ALIGN_CENTER, "zc:%d", mNetgame.zlib_cmp);
      }
...
void mwScreen::draw_demo_debug_overlay(int p, int &cx, int &cy)
{
   ...
   if (mSettings.overlay_grid[8][mLoop.show_debug_overlay]) // misc
   {
      ...
      al_draw_textf(mF.pr8, mC.pc[15], cx+1, cy+1, 0, "Time:[%d%%] ", mLoop.frame_num*100/mwDM.demo_mode_last_frame); cy+=9;
...
void mwScreen::draw_top_frame(int p)
{
   ...
   int tdy = 0;
   int tc = mC.get_contrasting_color(mPlayer.syn[p].color);
   ...
   if (mwD.SCREEN_W < 600) sprintf(msg,"Lv:%d Tm:%d En:%d ",            mLevel.play_level, mLoop.frame_num/40, mEnemy.num_enemy); // special case for narrow screens
...
void mwScreen::draw_bottom_frame(int p)
{
   ...
   if (mPlayer.syn[0].control_method == 1) // file play
   {
      ...
      sprintf(msg, "[%d%%] ", mLoop.frame_num*100/mwDM.demo_mode_last_frame);
      al_draw_text(mF.pr8, mC.pc[tc], bdx + ts, bdy, 0, msg);
      ts += strlen(msg)*8;
   }
}
