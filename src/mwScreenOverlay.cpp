...
void mwScreen::draw_demo_controls_overlay_play_stop_button(int xm, int x1, int x2, int ym, int y1, int y2, int r, int pc, int sc)
{
   ...
   {
      if (mWidget.mButton(xm, x1, x2, ym, y1, y2, r,  1,3,1,  pc+192,pc+80,15, 15,0, "Play")) mDemoMode.controls_paused = 0;
   }
   else
   {
      if (mWidget.mButton(xm, x1, x2, ym, y1, y2,  r,  1,3,1,  sc+192,sc+80,15, 15,0, "Stop")) mDemoMode.controls_paused = 1;
   }
}
...
void mwScreen::draw_demo_controls_overlay_bottom_line()
{
   ...
   // mode button fixed position in lower left corner
   ...
   x += ppc_length + 4; // update running position

   int pwd_length = 138;
   mWidget.mCheckBox(0,x,x+pwd_length,  2,h,y2, 0, mDemoMode.controls_pause_when_done, "Pause when done", 15, 15);

   x += pwd_length + 4; // update running position
   ...
   {
      if (mWidget.mButton(0,x,x+nrd_length,  2,h,y2,  1,  1,3,1,  12+192,12+80,15, 15,0, "Next Random Demo")) mDemoMode.continuous_random_next_level();
      x += nrd_length + 4; // update running position
   }

   if (mDemoMode.controls_mode == 0)
   {
      int tly = mDisplay.SCREEN_H - 11;
      x = 4 + draw_demo_controls_overlay_players_small(x, tly);

      mGmInfo.showTimeline(x, tly, mDisplay.SCREEN_W-1, tly+10, 9+160);
   }
}
...
//   mDemoMode.controls_mode = 1; // for testing

   if (mDemoMode.controls_mode == 2)
   {
      mDemoMode.controls_mode = 0;
   }
   ...
   if (mDemoMode.controls_mode == 1)
   {
      ...
      // sizes
      int speed_height = bts;
      int speed_width = 358;
      int transport_height = bts;
      int transport_width = 418;

      // frame x values (fixed, based on transport width)
      int x1 = BORDER_WIDTH;
      int w = transport_width + ls * 2;
      int x2 = x1 + w;

      // calc height of timeline
      int timeline_height = ls + (bts+ls)*mGmInfo.gmPlayerInfo.size() + 2;

      // frame y values
      int y2 = mDisplay.SCREEN_H - BORDER_WIDTH;
      int h = timeline_height + transport_height + speed_height + ls * 4;
      int y1 = y2 - h;
      ...
      // inner x starting position
      int xa = x1+ls;

      // inner y starting position
      int ya = y2-ls;

      // center speed controls
      int xs = xa + (transport_width - speed_width)/2;

      // draw speed controls
      draw_demo_controls_overlay_speed(xs, ya-speed_height, -1, speed_height);
      ya -= (speed_height + ls); // update running y position

      // draw transport controls
      draw_demo_controls_overlay_transport(xa, ya-transport_height, -1, transport_height);
      ya -= (transport_height + ls); // update running y position

      // draw timeline
      mGmInfo.showTimelineTracks(ya, xa+transport_width, bts, ls, xa);
   }
}

// button helper
bool bh(int &rx, int y1, int y2, int ls, int bc, const char* txt)
{
   ...
   if (mWidget.mButton(0, x1, x2,  0,y1,y2,  0,  1,3,1,  bc+192,bc+80,15, 15,0, txt)) return true;
   return false;
}


int mwScreen::draw_demo_controls_overlay_transport(int x1, int y1, int x2, int bts)
{
   ...
   if (bh(rx, y1, y2, s, bc, "start")) f  = 0;
   if (bh(rx, y1, y2, s, bc, "-60s" )) f -= 2400;
   if (bh(rx, y1, y2, s, bc, "-10s" )) f -= 400;
   if (bh(rx, y1, y2, s, bc, "-1s"  )) f -= 40;
   if (bh(rx, y1, y2, s, bc, "-1f"  )) f -= 1;

   int pc = 11; // play color
   int sc = 4;  // stop color
   draw_demo_controls_overlay_play_stop_button(0,rx,rx+40,  0,y1,y2,  0,  pc, sc);
   rx+=40+s;
...
void mwScreen::draw_demo_controls_overlay_speed(int x1, int y1, int x2, int bts)
{
   ...
   if (bh(rx, y1, y2, s, bc, "min" )) mLoop.frame_speed = 4;
   if (bh(rx, y1, y2, s, bc, "-50%")) sh(0, .5);
   if (bh(rx, y1, y2, s, bc, "-10%")) sh(0, .1);

   // display button in the middle with click to reset to defaults
   char msg[80];
   sprintf(msg, "Frame Speed:%d", mLoop.frame_speed);

   if (mWidget.mButton(0,rx,rx+128,  0,y1,y2,  0,  1,3,1,  bc+192,bc+80,15, 15,0, msg)) mLoop.frame_speed = 40;
   rx += 128+s;

   if (bh(rx, y1, y2, s, bc, "+50%")) sh(1, .5);
   if (bh(rx, y1, y2, s, bc, "+10%")) sh(1, .1);
   if (bh(rx, y1, y2, s, bc, "max" )) mLoop.frame_speed = 200;
