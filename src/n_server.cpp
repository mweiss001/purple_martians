...
void server_proc_player_drop(void)
{
   ...
   for (int p=1; p<NUM_PLAYERS; p++)   // server only; skip p[0]
      if (mPlayer.syn[p].control_method == 2)
      {
         ...
         if (mPlayer.loc[p].server_last_stak_rx_frame_num + 100 < mwPS.frame_num)
         {
            //printf("[%4d][%4d] drop p:%d \n", mwPS.frame_num, mPlayer.loc[p].server_last_stak_rx_frame_num, p);
...
void server_proc_cdat_packet(double timestamp)
{
   ...
   mPlayer.loc[p].server_game_move_sync = cdat_frame_num - mwPS.frame_num;
...
void server_lock_client(int p)
{
   ...
   {
      mwGMA.add_game_move(mwPS.frame_num + 4, 1, p, mPlayer.syn[p].color);
...
void server_proc_stak_packet(double timestamp)
{
   ...
   mPlayer.loc[p].server_last_stak_rx_frame_num = mwPS.frame_num;
...
void server_proc_cjon_packet(int who)
{
   ...
   else // empty player slot found, proceed with join
   {
      ...
      mPlayer.loc[cn].server_last_stak_rx_frame_num = mwPS.frame_num + 200;
