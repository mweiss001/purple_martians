...
int mwEnemy::enemy_data(int x_pos, int y_pos)
{
   sort_enemy();
...
void mwEnemy::rectangle_with_diagonal_lines(float x1, float y1, float x2, float y2, int spacing, int frame_color, int line_color, int clip_mode)
{
...
int mwEnemy::is_player_in_trigger_box(int x1, int y1, int x2, int y2)
{
   for (int p=0; p<NUM_PLAYERS; p++)
      if ((players[p].active) && (!players[p].paused))
      {
...
void mwEnemy::enemy_podzilla(int e)
{
...
void mwEnemy::enemy_vinepod(int e)
{
   if (Ei[e][31]) // hit
   {
      int ek = 0;
...
void mwEnemy::bouncer_cannon_common(int e)
{
...
//----------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------
void mwEnemy::enemy_cannon(int e)
{
   enemy_player_hit_proc(e);
...
void mwEnemy::enemy_bouncer(int e)
{
   if (Ei[e][31]) // hit
   {
      enemy_killed(e);
...
void mwEnemy::enemy_archwagon(int e)
{
   int EXint = Ef[e][0];
   int EYint = Ef[e][1];
...
void mwEnemy::walker_archwagon_common(int e)
{
   int EXint = Ef[e][0];
   int EYint = Ef[e][1];
...
void mwEnemy::enemy_block_walker(int e)
{
   int EXint = Ef[e][0];
   int EYint = Ef[e][1];
...
void mwEnemy::seek_set_xyinc(int e)
{
   int p = find_closest_player(e);
   float xlen = players[p].x - Ef[e][0];
   float ylen = players[p].y - Ef[e][1];
