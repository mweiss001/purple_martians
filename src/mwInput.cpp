...
void mwInput::function_key_check(void)
{
   ...
   if ((mLoop.level_editor_running) || (mLoop.state[1] == PM_PROGRAM_STATE_MAIN_GAME_LOOP) || (mLoop.state[1] == PM_PROGRAM_STATE_DEMO_RECORD) || (mDemoMode.play_mode)) // only do these when game is running or in level editor
   {
      // zoom controls
