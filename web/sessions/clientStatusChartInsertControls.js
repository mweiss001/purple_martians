...
export function setupClientStatusChartInsertControls(parentContainer)
{
   const container = createControlGroup('Client Status Insert Controls', 'clientStatusControlsContainer', '#0066FF');
...
//   container.style.height = '1660px';
   ...
   setupLabelSpan(container, 'sicsb_time_avpSpan', "Avg per row:", '');
...
export function updateClientStatusChartInsertControls(data)
{
   if (!data) return;
   ...
   const avg = (data[0].sicsb_time_avg/1000).toFixed(1);
   const max = (data[0].sicsb_time_max/1000).toFixed(1);
   const avg_per = ((data[0].sicsb_time_avg/data[0].sicsb_size_actual)/1000).toFixed(2);

   setSpanInnerHTML('sicsb_time_avgSpan', avg);
   setSpanInnerHTML('sicsb_time_maxSpan', max);
   setSpanInnerHTML('sicsb_time_avpSpan', avg_per);
